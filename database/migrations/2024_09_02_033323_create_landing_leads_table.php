...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('landing_leads', function (Blueprint $table) {
            ...
            $table->string('identification','18')->notNullable()->comment('CURP');
            $table->string('first_name','255')->notNullable()->comment('User_s First name');
            $table->string('last_name','255')->notNullable()->comment('User_s Last name'); //
            $table->string('phone','30')->notNullable()->comment('Phone number');
            $table->string('email','255')->notNullable()->comment('User Email address');
            $table->date('birthdate')->comment('Date of birth');
            $table->enum('gender', ['h', 'm'])->comment('Gender: h = male, m = female');
            $table->string('subject','255')->notNullable()->comment('Subject');
            $table->string('account','255')->notNullable()->comment('Sponsor_s name');
